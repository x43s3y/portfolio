...
let downButtonEl = document.getElementsByClassName('down-button')[0];
...
for (let button of lang_buttons) {
	button.addEventListener('click', () => {
		...
		if (button.className.includes('active')) {
			...
			reverseDot();
			setTimeout(() => {
				downButtonEl.classList.remove('gone');
		...
		setTimeout(() => {
			downButtonEl.classList.add('gone');
		}, 100);

		//activate animation and remove buttons
		setTimeout(() => {
